...
def biblioteca(request):
    biblio = Joc.objects.filter(usuari=request.user)
    ctx={"biblio":biblio}
    return render(request,"menu_usuari.html",ctx)
...
def afegir_al_carritu(request, id_joc):
    ...
    nova_comanda = Comanda.objects.create(carro = carritu, joc = joc_demanat, preu = joc_demanat.preu, quantitat = 1)
    carritu.preu_total += joc_demanat.preu
    carritu.save()
